...
export class Tree implements ITree {
  ...
  public delete(dirChain: string[], isMove: boolean = false): void {
    if (!isMove && !this.checkParentsChain(dirChain, CommandsEnum.DELETE)) { return; }
  ...
  public move(dirChain: string[], dirTo: string): void {
    if (!this.checkParentsChain(dirChain, CommandsEnum.MOVE)) { return; }
  ...
  private checkParentsChain(dirChain: string[], type: string): boolean {
    let isCorrectPath: boolean = true;

    dirChain.forEach((dir, index) => {
      if (!isCorrectPath) { return; }
      const foundedDir = this.search(dir);
      const parentNode = typeof foundedDir !== 'string' && (foundedDir as TreeNode).getParent();
      ...
      if (foundedDir === this.searchError ||
        ...
        isCorrectPath = false;
        this.logger.log(`Cannot ${type} ${dirChain.join('/')} - ${dir} does not exist`);
      }
